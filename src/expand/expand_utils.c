...
/*   Updated: 2026/01/25 16:30:15 by cpinas           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
char	*append_expanded_var(char *result, char *word, int *i, t_shell *shell)
{
	char	*value;
	char	*tmp;
	int		consumed;

	value = get_var_value(&word[*i + 1], &consumed, shell);
	if (!value)
